        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{$t("medical_service.hospital")}}</li>
            ...
            <div class="spas-list">
                ...
                <div class="row">
                    <div class="col-sm-4" v-for="spa in spas" :key="spa.id">
                        <app-single-hospital :hospital="spa"></app-single-hospital>
                    </div>
                </div>
...
import SingleHospital from '~/components/Vendor/SingleHospital';

export default {
    components: {
        appSingleHospital: SingleHospital
    },
    data() {
        return {
            spas: [],
    ...
    async fetch() {
        let spasData = await axios.get('/my/hospitals');
            ...
            if(Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) < 1 &&  Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) > -10) {
                 if(this.allowToGetMore && !this.noMore) { 
                     ...
                     try {
                        ...
                        let spasData = await axios.get(`my/hospitals/skip/${this.spas.length}/take/6`);
                        console.log('spas >> ', spasData.data.data);
