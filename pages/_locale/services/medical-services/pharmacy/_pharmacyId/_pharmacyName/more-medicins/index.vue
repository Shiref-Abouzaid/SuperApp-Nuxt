        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><nuxt-link :to="{name:'locale-services-medical-services-pharmacies'}" >{{ $t("subnav.pharmacy") }}</nuxt-link></li>
                    <li class="breadcrumb-item"><nuxt-link :to="{name:'locale-services-medical-services-pharmacy-pharmacyId-pharmacyName', params:{pharmacyName:$route.params.pharmacyName, pharmacyId:$route.params.pharmacyId}}" >{{$route.params.pharmacyName}}</nuxt-link></li>
...
export default {
    ...
    async fetch() {
        try {
            let resData = await axios.get(`/my/products/store/${this.$route.params.pharmacyId}/type/pharmacy/skip/0/take/5`)
            ...
            if(Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) < 1 &&  Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) > -10) {
                 if(this.allowToGetMore && !this.noMore) { 
                     ...
                     try {
                        ...
                        let resData = await axios.get(`/my/products/store/${this.$route.params.pharmacyId}/type/pharmacy/skip/${this.products.length}/take/6`)
