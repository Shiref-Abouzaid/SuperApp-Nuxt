        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{$t("medical_service.pharmacies")}}</li>
                </ol>
            </nav>
            ...
            <div class="spas-list">
                ...
                <div class="row">
                    <div class="col-sm-4" v-for="spa in spas" :key="spa.id">
                        <app-single-pharmacy :pharmacy="spa"></app-single-pharmacy>
                    </div>
                </div>
...
import SinglePharamcy from '~/components/Vendor/SinglePharamcy';

export default {
    components: {
        appSinglePharmacy: SinglePharamcy
    },
    data() {
        return {
            spas: [],
    ...
    async fetch() {
        let spasData = await axios.get('/my/pharmacies/random/6');
        console.log('pharmacies >> ', spasData.data.data);
            ...
            if(Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) < 1 &&  Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) > -10) {
                 if(this.allowToGetMore && !this.noMore) { 
                     ...
                     try {
                        ...
                        let spasData = await axios.get(`my/pharmacies/skip/${this.spas.length}/take/6`);
