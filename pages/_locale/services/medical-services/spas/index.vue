        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{$t("medical_service.spa")}}</li>
                </ol>
            </nav>
            ...
            <div class="spas-list">
                ...
                <div class="row">
                    <div class="col-sm-4" v-for="spa in spas" :key="spa.id">
                        <app-single-spa :spa="spa"></app-single-spa>
                    </div>
                </div>
...
import SingleSpa from '~/components/Vendor/SingleSpa';

export default {
    components: {
        appSingleSpa: SingleSpa
    },
    data() {
        return {
            spas: [],
    ...
    async fetch() {
        try {
            let spasData = await axios.get('/my/spa/skip/0/take/6');
            ...
            if(Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) < 1 &&  Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) > -10) {
                 if(this.allowToGetMore && !this.noMore) { 
                     ...
                     try {
                        ...
                        let spasData = await axios.get(`my/spa/skip/${this.spas.length}/take/6`);
                        console.log('spas >> ', spasData.data.data);
