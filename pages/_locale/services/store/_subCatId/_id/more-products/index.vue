...
export default {
    ...
    async fetch() {
        try {
            let resData = await axios.get(`my/category/${this.$route.params.id}/products/skip/0/take/6`)
            ...
            if(Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) < 1 &&  Math.floor(windowScrollY - (windowScrollHeight-widnowHeight)) > -10) {
                 if(this.allowToGetMore && !this.noMore) { 
                     ...
                     try {
                        ...
                        let resData = await axios.get(`my/category/${this.$route.params.id}/products/skip/${this.products.length}/take/6`)
