        ...
        <div class="container">
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ $route.params.auctionName }}</li>
                </ol>
            </nav>
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    ...
                    <div class="ended" v-else>
                        ...
                        <p class="text-center" v-if="winnedUser">Owned by <span class="name">{{(winnedUser.winingUser && winnedUser.winingUser.first_name) ||(winnedUser.user && winnedUser.user.first_name)}} {{(winnedUser.winingUser && winnedUser.winingUser.last_name) ||(winnedUser.user && winnedUser.user.last_name)}}</span></p>
                        <p class="text-center" v-if="winnedUser">with <span class="price">{{currSymbol}}{{winnedUser.winingPrice || winnedUser.price}}</span></p>
                    </div>
...
export default {
    ...
    computed: {
        ...
        deals() {
            return this.$store.getters['realtime/auctionsOffers'][this.$route.params.auctionId]
        },
        token() {
    ...
    async created() {
        ...
        try {
            ...
            //remove all deals, because when getting the auction, we get all the deals
            this.$store.commit('realtime/resetAuctionOffers', this.$route.params.auctionId);
            this.$store.commit('realtime/setAuctionOffers', {
                id: this.$route.params.auctionId,
                deals: this.auction.makeDeal
            })
    ...
    mounted() {
        ...
        // if(process.client) {
            ...
            this.pusher = new Pusher(`dpctgdv35p33t63d3eva`, {
                ...
                wsPort: 6001,
                authEndpoint: 'https://developers.api.tbdm.net/v-1872020/' + this.locale + '/my/auction/' + this.$route.params.auctionId + '/auth/presence',
                auth: {
                    headers: {
            ...
            this.channelName = 'presence-auction.' + this.$route.params.auctionId;
            let auctionChannel = this.pusher.subscribe('presence-auction.' + this.$route.params.auctionId);

            auctionChannel.bind('App\\Events\\AuctionBidCreated', (currentBid) => {
                console.log('got data in single auction >>>>> ', currentBid)
            ...
            auctionChannel.bind('App\\Events\\AuctionClosed', (closed) => {
                console.log('got data closed event single >>>>> ', closed)
                this.winnedUser = closed;
                clearInterval(this.interval);
    ...
    methods: {
        async makeBid() {
            ...
            try {
                ...
                let sendBid = await axios.post('/my/auctions/' + this.$route.params.auctionId + '/bids', bid);
                console.log('bid >> ', sendBid)
            } catch (error) {
        ...
        async onNo(deal) {
            console.log('deal!', deal);
            this.$store.commit('realtime/removeDeal', deal);
            try {
                let userId = this.$store.getters['auth/userId'];
                let status = {
                    status: 'rejected'
    ...
    beforeDestroy() {
        console.log('destroyed >> ', this.interval, this.channelName);
        clearInterval(this.interval);
